...
export default function ServicesPage() {
  ...
  const handleDelete = async (serviceId: string) => {
    if (!confirm(t('common.confirmDelete', 'Are you sure you want to delete this service?'))) return
  ...
  return (
    <div>
      <div className="mb-4 lg:mb-6">
        <h1 className="text-2xl lg:text-3xl font-bold mb-2 text-gray-900 dark:text-gray-100">{t('services.title')}</h1>
        <p className="text-sm lg:text-base text-gray-700 dark:text-gray-400">{t('services.subtitle')}</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-2 sm:gap-3 mb-4 lg:mb-6">
        <Button onClick={() => setShowForm(true)} className="w-full sm:w-auto justify-center sm:justify-start">
          ...
          <span className="text-xs">{t('services.addService')}</span>
        </Button>
        <select
        ...
        >
          <option value="all">{t('services.allBuildings')}</option>
