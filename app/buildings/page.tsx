...
export default function BuildingsPage() {
  ...
  const getStatusLabel = (progress: number) => {
    if (progress === 0) return { label: t('buildings.planning'), color: 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300' }
    if (progress < 50) return { label: t('buildings.inProgress'), color: 'bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-300' }
    if (progress < 100) return { label: t('buildings.inProgress'), color: 'bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-300' }
    return { label: t('buildings.completed'), color: 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-300' }
  }
  ...
  return (
    <div>
      <div className="mb-4 lg:mb-6">
        <h1 className="text-2xl lg:text-3xl font-bold mb-2 text-gray-900 dark:text-gray-100">{t('buildings.title')}</h1>
        <p className="text-sm lg:text-base text-gray-700 dark:text-gray-400">{t('buildings.subtitle')}</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-2 sm:gap-3 mb-4 lg:mb-6">
        <Button className="w-full sm:w-auto justify-center sm:justify-start">
          ...
          <span className="text-xs">{t('buildings.addBuilding')}</span>
        </Button>
        <select
        ...
        >
          <option value="all">{t('buildings.allProjects')}</option>
            ...
            return (
              ...
              >
                ...
                <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-1">
                  {building.building_code === 'الرئيسية' || building.building_code === 'Main' ? t('buildings.mainBuilding') : `${t('buildings.building')} ${building.building_code}`}
                </h3>
                <p className="text-sm text-gray-700 dark:text-gray-400 mb-4">{building.project?.name || t('common.noProject')}</p>

                <div className="mb-4">
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-gray-700 dark:text-gray-400">{t('buildings.services')}:</span>
                    <span className="font-medium text-gray-900 dark:text-gray-100">{building.services_count} {t('buildings.services')}</span>
                  </div>
                  <div className="flex justify-between text-sm mb-2">
                    <span className="text-gray-700 dark:text-gray-400">{t('buildings.progress')}:</span>
                ...
                <div className="flex gap-2">
                  <Link href={`/buildings/${building.id}`} className="flex-1">
                    <Button variant="outline" className="w-full text-xs sm:text-sm">
                      ...
                      <span>{t('buildings.fromProgress')}</span>
                    </Button>
                  </Link>
                  <Link href={`/buildings/${building.id}`} className="flex-1">
                    <Button variant="outline" className="w-full text-xs sm:text-sm">
                      <Wrench size={14} className="ml-1.5" />
                      <span>{t('buildings.services')}</span>
                    </Button>
                  </Link>
                </div>
              </div>
