 ...
 * 
 * NOTE: This module uses browser-only APIs (document, window, html2canvas, jsPDF)
 * It should ONLY be called from client-side components ('use client')
 * 
 * It will work on Vercel because:
 * 1. It's only called from client components
 * 2. It checks for browser environment before execution
 * 3. Next.js handles client-side code correctly on Vercel
 */
